...
// park miller carta uniform prng

parkMiller = function(seed) {
  if(!seed)
    seed = 1;
  if(typeof seed !== 'number' || seed < 1 || seed >= 2147483647 || seed % 1)
    throw "Bad seed.";

  return function(raw) {
    seed = (seed * 16807) % 2147483647;
    return raw ? seed : seed / 2147483647;
  };
};


var RAND = parkMiller(0x4723235);
...
(function() {
  ...
  // a basic fractional brownian motion thing
  function brownian( noise, x, y, res ) {
    var val = 1, sum = 0, ttl = 0;

    while( res < 0.05 ) {
      ttl += val;
      sum += noise( x * res, y * res ) * val;
      val /= 2;
      res *= 2;
    }

    return sum / ttl * 0.5 + 0.5;
  }


  //
  function terraces( noise, x, y, res ) {
    var val = brownian( noise, x, y, res )
      , vres = noise( x * res / 8, y * res / 8 ) / 4 + 0.25
      , vmod = val % vres
      , vrel = ( vres - vmod ) / vres
      , amount;

    if( vrel > 0.001 )
      return val - vmod * vrel;
  ...
  //
  function polarize( noise, x, y, res ) {
    var val = terraces( noise, x, y, res )
      , amount = noise( x * res / 5, y * res / 5 )
      , pol = val * 2 - 1;
    ...
    pol = pol / 2 + 0.5;
    amount *= amount;
    return amount * pol + ( 1 - amount ) * val;
  }


  // the usual fractal bs
  // function hills( noise, map ) {
  //   var x, y, i = 0
  //     , n = Math.sqrt( map.length )
  //     , BASE = 256
  //     , offset = 0xffff;

  //   for( x = 0; x < n; x ++ )
  //     for( y = 0; y < n; y ++ )
  //       map[ i ++ ] = polarize( noise, x + offset, y - offset, 0.00390625 / 2 )
  //         * BASE;
  // }
