...
(function() {
  ...
  function updateTracker(id, time) {
    ...
    if ('projectCourse' in tracker) {
      ...
      // currently they're too big if closely observed
      if (total > 0.4 || !testIfCourseRelevant(tracker, 10, 0.3))
  ...
  GAME.sendTrackerUpdate = function(id, data) {
    ...
    var tracker = trackers[id];
    if (tracker && 'projectCourse' in tracker)
