...
(function() {
  ...
  function updateTracker(id, time) {
    ...
    if ('simApply' in tracker) {
      ...
      if (total > 1.9 || !testIfCourseRelevant(tracker, 1.5, 0.025)) {
        GAME.log(Date.now(), total);
  ...
  GAME.upsertTracker = function(id, data, makeModel) {
    ...
    // update
    else if (GAME.isNewerRev(data.rev, tracker.$t_rev)) {
      tracker.$t_rev = data.rev;
      tracker.$t_dat = data;
      tracker.$t_tim = 0;
    }
  ...
  function testIfCourseRelevant(tracker, dthresh, rthresh) {
    ...
    vTmp2.set(0, 0, 1);
    vTmp2.applyQuaternion(tracker.quaternion);
  ...
  GAME.sendTrackerUpdate = function(id, data) {
    ...
    // next rev
    data.rev = GAME.nextRev(data.rev || 0);
