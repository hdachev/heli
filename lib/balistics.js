...
(function() {
  ...
  GAME.subscribe(GAME.TRACKER_UPDATED, function(id, data, tracker) {
    var x = trackers.indexOf(tracker)
      , firing = data.firing
      , ok = Array.isArray(firing);

    // Map ids to weapons.
    if (ok)
      firing = firing.map(function(w) {
  ...
  GAME.defineRotaryCannon = function(name, spec) {
    ...
    , applyControl: function(tracker, gamepad, time) {
        ...
        if (fire) {
          ...
          // Generate heat.
          tracker.$b_heat = heat + time;

          // Open fire.
          GAME.trackerSADD(tracker, 'firing', name);
        }
        else {
          ...
          // Cease fire.
          GAME.trackerSREM(tracker, 'firing', name);
        }
      }
    };
...
*/
  ...
  GAME.defineRotaryCannon('vulcan', {
    cal: 0.02
  , vel: 1050
  , rof: 6600
  , ammo: 'ap'
